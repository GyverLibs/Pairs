...
### Доступ через []
В библиотеке реализован доступ к значениям через квадратные скобки на чтение и запись. Это удобно, но есть особенность: при чтении по ключу, которого нет в базе - будет создано новое поле с этим ключом и пустым знанием! При использовании функции `get()` этого не происходит.
...
### AnyText
Библиотека наследует класс `AnyText` из библиотеки [StringUtils](https://github.com/GyverLibs/StringUtils), полное описание смотри там. Любой вызов `pairs[key]` конвертируется в тип `AnyText`:
...
#### На выход
`AnyText` автоматически конвертируется в любые типы данных, что позволяет получать значения напрямую:
```cpp
int i = data["key"];
float f = data["key"];
String s = data["key"];
...
bool set(int idx, AnyValue value);      // установить по индексу

Pair_t get(AnyText key);                // получить по ключу
Pair_t get(int idx);                    // получить по индексу
